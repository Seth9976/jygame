...
namespace System.Xml.Serialization
{
	...
	public class XmlSchemaImporter : SchemaImporter
	{
		...
		public XmlMembersMapping ImportAnyType(XmlQualifiedName typeName, string elementName)
		{
			...
			this.ImportSequenceContent(typeName, classMap, ((XmlSchemaSequence)xmlSchemaComplexType.Particle).Items, codeIdentifiers, false, ref isMixed);
		...
		public XmlTypeMapping ImportDerivedTypeMapping(XmlQualifiedName name, Type baseType, bool baseTypeCanBeIndirect)
		{
			...
			else if (!this.LocateElement(name, out xmlQualifiedName, out xmlSchemaType))
			{
				return null;
			}
			XmlTypeMapping xmlTypeMapping = this.GetRegisteredTypeMapping(xmlQualifiedName, baseType);
			...
			xmlTypeMapping = this.CreateTypeMapping(xmlQualifiedName, SchemaTypes.Class, name);
			if (xmlSchemaType != null)
			{
				xmlTypeMapping.Documentation = this.GetDocumentation(xmlSchemaType);
				this.RegisterMapFixup(xmlTypeMapping, xmlQualifiedName, (XmlSchemaComplexType)xmlSchemaType);
			}
			else
			{
				ClassMap classMap = new ClassMap();
				...
				this.AddTextMember(xmlQualifiedName, classMap, codeIdentifiers);
			}
		...
		private void SetMapBaseType(XmlTypeMapping map, Type baseType)
		{
			...
			foreach (object obj in classMap.AllMembers)
			{
				...
				classMap2.AddMember(xmlTypeMapMember);
			}
			foreach (object obj2 in xmlTypeMapping.DerivedTypes)
			{
				...
				{
					XmlTypeMapMember xmlTypeMapMember2 = (XmlTypeMapMember)obj3;
					classMap2.AddMember(xmlTypeMapMember2);
				}
			}
		}
		...
		public XmlMembersMapping ImportMembersMapping(XmlQualifiedName name)
		{
			...
			XmlMemberMapping[] array = this.ImportMembersMappingComposite(xmlSchemaComplexType, name);
			return new XmlMembersMapping(name.Name, name.Namespace, array);
		}
		...
		public XmlMembersMapping ImportMembersMapping(XmlQualifiedName[] names)
		{
			...
			for (int i = 0; i < names.Length; i++)
			{
				...
				TypeData elementTypeData = this.GetElementTypeData(xmlQualifiedName, xmlSchemaElement, names[i], out xmlTypeMapping);
				array[i] = this.ImportMemberMapping(xmlSchemaElement.Name, xmlQualifiedName.Namespace, xmlSchemaElement.IsNillable, elementTypeData, xmlTypeMapping);
			}
			this.BuildPendingMaps();
		...
		{
			XmlSchemaType xmlSchemaType = ((XmlSchemaType)this.schemas.Find(typeName, typeof(XmlSchemaComplexType))) ?? ((XmlSchemaType)this.schemas.Find(typeName, typeof(XmlSchemaSimpleType)));
			return this.ImportTypeCommon(typeName, typeName, xmlSchemaType, true);
		}
		...
		internal XmlMembersMapping ImportEncodedMembersMapping(string name, string ns, SoapSchemaMember member)
		{
			...
			XmlMemberMapping[] array = this.ImportMembersMappingComposite(xmlSchemaComplexType, member.MemberType);
			return new XmlMembersMapping(name, ns, array);
		}
		...
		private XmlMemberMapping[] ImportMembersMappingComposite(XmlSchemaComplexType stype, XmlQualifiedName refer)
		{
			...
			CodeIdentifiers codeIdentifiers = new CodeIdentifiers();
			this.ImportParticleComplexContent(refer, classMap, xmlSchemaSequence, codeIdentifiers, false);
			this.ImportAttributes(refer, classMap, stype.Attributes, stype.AnyAttribute, codeIdentifiers);
			this.BuildPendingMaps();
			int num = 0;
		...
		public XmlTypeMapping ImportTypeMapping(XmlQualifiedName name)
		{
			...
			XmlQualifiedName xmlQualifiedName;
			XmlSchemaType xmlSchemaType;
			if (!this.LocateElement(xmlSchemaElement, out xmlQualifiedName, out xmlSchemaType))
			{
				throw new InvalidOperationException(string.Format("'{0}' is missing.", name));
			}
			return this.ImportTypeCommon(name, xmlQualifiedName, xmlSchemaType, xmlSchemaElement.IsNillable);
		}
		...
		{
			XmlSchemaElement xmlSchemaElement = (XmlSchemaElement)this.schemas.Find(name, typeof(XmlSchemaElement));
			return this.LocateElement(xmlSchemaElement, out qname, out stype);
		}
		...
		private XmlTypeMapping ImportClass(XmlQualifiedName name)
		{
			...
			XmlSchemaComplexType xmlSchemaComplexType = this.schemas.Find(name, typeof(XmlSchemaComplexType)) as XmlSchemaComplexType;
			return this.CreateClassMap(name, xmlSchemaComplexType, new XmlQualifiedName(xmlTypeMapping.ElementName, xmlTypeMapping.Namespace));
		}
		...
		private XmlTypeMapping ImportClassComplexType(XmlQualifiedName typeQName, XmlSchemaComplexType stype, XmlQualifiedName root)
		{
			...
			{
				TypeData typeData;
				ListMap listMap = this.BuildArrayMap(typeQName, stype, out typeData);
				if (listMap != null)
				{
		...
		private void ImportAttributes(XmlQualifiedName typeQName, ClassMap cmap, XmlSchemaObjectCollection atts, XmlSchemaAnyAttribute anyat, CodeIdentifiers classIds)
		{
			...
			foreach (XmlSchemaObject xmlSchemaObject in atts)
			{
				if (xmlSchemaObject is XmlSchemaAttribute)
				{
					...
					string text;
					XmlSchemaAttribute refAttribute = this.GetRefAttribute(typeQName, xmlSchemaAttribute, out text);
					...
					xmlTypeMapMemberAttribute.Namespace = text;
		...
		private ListMap BuildArrayMap(XmlQualifiedName typeQName, XmlSchemaComplexType stype, out TypeData arrayTypeData)
		{
			if (this.encodedFormat)
			{
				...
				if (xmlSchemaAttribute != null)
				{
					...
					return this.BuildEncodedArrayMap(text + text3, text2, out arrayTypeData);
				}
				else
				{
			...
			else
			{
				ClassMap classMap = new ClassMap();
				CodeIdentifiers codeIdentifiers = new CodeIdentifiers();
				this.ImportParticleComplexContent(typeQName, classMap, stype.Particle, codeIdentifiers, stype.IsMixed);
		...
		private ListMap BuildEncodedArrayMap(string type, string ns, out TypeData arrayTypeData)
		{
			...
			if (text.IndexOf("[") != -1)
			{
				ListMap listMap2 = this.BuildEncodedArrayMap(text, ns, out typeData);
				int num2 = text.Split(new char[] { '[' }).Length - 1;
				string arrayName = TypeTranslator.GetArrayName(type, num2);
				XmlQualifiedName xmlQualifiedName = new XmlQualifiedName(arrayName, ns);
				XmlTypeMapping xmlTypeMapping = this.CreateArrayTypeMapping(xmlQualifiedName, typeData);
				xmlTypeMapping.ObjectMap = listMap2;
			}
			else
			{
		...
		private void ImportSequenceContent(XmlQualifiedName typeQName, ClassMap cmap, XmlSchemaObjectCollection items, CodeIdentifiers classIds, bool multiValue, ref bool isMixed)
		{
			foreach (XmlSchemaObject xmlSchemaObject in items)
			{
				if (xmlSchemaObject is XmlSchemaElement)
				{
					...
					XmlTypeMapping xmlTypeMapping;
					TypeData typeData = this.GetElementTypeData(typeQName, xmlSchemaElement, null, out xmlTypeMapping);
					string text;
					XmlSchemaElement refElement = this.GetRefElement(typeQName, xmlSchemaElement, out text);
					if (xmlSchemaElement.MaxOccurs == 1m && !multiValue)
					{
						...
						xmlTypeMapMemberElement.ElementInfo.Add(this.CreateElementInfo(text, xmlTypeMapMemberElement, refElement.Name, typeData, refElement.IsNillable, refElement.Form, xmlTypeMapping));
					...
					else
					{
						...
						xmlTypeMapMemberFlatList.ElementInfo.Add(this.CreateElementInfo(text, xmlTypeMapMemberFlatList, refElement.Name, typeData, refElement.IsNillable, refElement.Form, xmlTypeMapping));
						xmlTypeMapMemberFlatList.ListMap.ItemInfo = xmlTypeMapMemberFlatList.ElementInfo;
						cmap.AddMember(xmlTypeMapMemberFlatList);
					}
				}
		...
		private void ImportChoiceContent(XmlQualifiedName typeQName, ClassMap cmap, XmlSchemaChoice choice, CodeIdentifiers classIds, bool multiValue)
		{
			...
			if (flag)
			{
				...
				for (int j = 0; j < xmlTypeMapElementInfoList.Count; j++)
				{
					...
					string text = ((!flag3) ? xmlTypeMapElementInfo2.ElementName : (xmlTypeMapElementInfo2.Namespace + ":" + xmlTypeMapElementInfo2.ElementName));
					string text2 = codeIdentifiers.AddUnique(CodeIdentifier.MakeValid(xmlTypeMapElementInfo2.ElementName), xmlTypeMapElementInfo2);
					array[j] = new EnumMap.EnumMapMember(text, text2);
				}
		...
		private bool ImportChoices(XmlQualifiedName typeQName, XmlTypeMapMember member, XmlTypeMapElementInfoList choices, XmlSchemaObjectCollection items)
		{
			...
			foreach (XmlSchemaObject xmlSchemaObject in items)
			{
				...
				if (xmlSchemaObject2 is XmlSchemaElement)
				{
					...
					XmlTypeMapping xmlTypeMapping;
					TypeData elementTypeData = this.GetElementTypeData(typeQName, xmlSchemaElement, null, out xmlTypeMapping);
					string text;
					XmlSchemaElement refElement = this.GetRefElement(typeQName, xmlSchemaElement, out text);
					choices.Add(this.CreateElementInfo(text, member, refElement.Name, elementTypeData, refElement.IsNillable, refElement.Form, xmlTypeMapping));
		...
		private void ImportComplexContent(XmlQualifiedName typeQName, XmlTypeMapping map, XmlSchemaComplexContent content, CodeIdentifiers classIds, bool isMixed)
		{
			...
			{
				xmlQualifiedName = ((XmlSchemaComplexContentRestriction)content.Content).BaseTypeName;
			}
			if (xmlQualifiedName == typeQName)
			{
				throw new InvalidOperationException(string.Concat(new string[] { "Cannot import schema for type '", typeQName.Name, "' from namespace '", typeQName.Namespace, "'. Redefine not supported" }));
			}
			XmlTypeMapping xmlTypeMapping = this.ImportClass(xmlQualifiedName);
			this.BuildPendingMap(xmlTypeMapping);
			ClassMap classMap2 = (ClassMap)xmlTypeMapping.ObjectMap;
			foreach (object obj in classMap2.AllMembers)
			{
			...
			foreach (object obj in this.schemas)
			{
				...
				foreach (XmlSchemaObject xmlSchemaObject in xmlSchema.Items)
				{
					...
					if (xmlSchemaComplexType != null && xmlSchemaComplexType.ContentModel is XmlSchemaComplexContent)
					{
						...
						{
							xmlQualifiedName = ((XmlSchemaComplexContentRestriction)xmlSchemaComplexType.ContentModel.Content).BaseTypeName;
						}
		...
		private XmlTypeMapping ImportClassSimpleType(XmlQualifiedName typeQName, XmlSchemaSimpleType stype, XmlQualifiedName root)
		{
			if (this.CanBeEnum(stype))
			{
				...
				for (int i = 0; i < xmlSchemaSimpleTypeRestriction.Facets.Count; i++)
				{
					...
					string text = codeIdentifiers.AddUnique(CodeIdentifier.MakeValid(xmlSchemaEnumerationFacet.Value), xmlSchemaEnumerationFacet);
					array[i] = new EnumMap.EnumMapMember(xmlSchemaEnumerationFacet.Value, text);
		...
		private bool CanBeArray(XmlQualifiedName typeQName, XmlSchemaParticle particle, bool multiValue)
		{
			...
			else
			{
				...
				if (particle is XmlSchemaChoice)
				{
					...
					return this.CheckChoiceType(typeQName, particle, arrayList, ref multiValue) && multiValue;
				}
				return false;
			}
		}
				...
				if (!(particle is XmlSchemaAny))
				{
					if (particle is XmlSchemaSequence)
					{
						...
						foreach (XmlSchemaObject xmlSchemaObject in xmlSchemaSequence.Items)
						{
							...
							if (!this.CheckChoiceType(typeQName, xmlSchemaParticle, types, ref multiValue))
							{
								return false;
							}
						}
					...
					if (particle is XmlSchemaChoice)
					{
						foreach (XmlSchemaObject xmlSchemaObject2 in ((XmlSchemaChoice)particle).Items)
						{
							...
							if (!this.CheckChoiceType(typeQName, xmlSchemaParticle2, types, ref multiValue))
							{
								return false;
							}
						}
		...
		private XmlTypeMapping CreateTypeMapping(XmlQualifiedName typeQName, SchemaTypes schemaType, XmlQualifiedName root)
		{
			...
			{
				text2 = root.Name;
				text3 = root.Namespace;
			}
			else
			{
				text2 = typeQName.Name;
				text3 = string.Empty;
			}
			XmlTypeMapping xmlTypeMapping = new XmlTypeMapping(text2, text3, typeData, typeQName.Name, typeQName.Namespace);
			xmlTypeMapping.IncludeInSchema = true;
		...
		private TypeData GetElementTypeData(XmlQualifiedName typeQName, XmlSchemaElement elem, XmlQualifiedName root, out XmlTypeMapping map)
		{
			...
			{
				typeData = this.GetTypeData(elem.SchemaType, typeQName, elem.Name, flag, root);
			}
		...
		private TypeData GetAttributeTypeData(XmlQualifiedName typeQName, XmlSchemaAttribute attr)
		{
			...
			return this.GetTypeData(attr.SchemaType, typeQName, attr.Name, flag, null);
		}
		...
		private TypeData GetTypeData(XmlSchemaType stype, XmlQualifiedName typeQNname, string propertyName, bool sharedAnnType, XmlQualifiedName root)
		{
			...
			XmlQualifiedName xmlQualifiedName = new XmlQualifiedName(text, typeQNname.Namespace);
			XmlTypeMapping xmlTypeMapping = this.ImportType(xmlQualifiedName, stype, root);
			if (sharedAnnType)
			{
		...
		private XmlTypeMapping GetTypeMapping(TypeData typeData)
		{
			...
			if (typeData.IsListType)
			{
				...
				xmlTypeMapping.ObjectMap = new ListMap
				{
					...
					ItemInfo = { this.CreateElementInfo(typeMapping.Namespace, null, typeData.ListItemTypeData.XmlType, typeData.ListItemTypeData, false, XmlSchemaForm.None) }
				};
		...
		private XmlSchemaAttributeGroup FindRefAttributeGroup(XmlQualifiedName refName)
		{
			...
			foreach (XmlSchemaObject xmlSchemaObject in xmlSchemaAttributeGroup.Attributes)
			{
				...
				{
					throw new InvalidOperationException(string.Concat(new string[] { "Cannot import attribute group '", refName.Name, "' from namespace '", refName.Namespace, "'. Redefine not supported" }));
				}
			}
